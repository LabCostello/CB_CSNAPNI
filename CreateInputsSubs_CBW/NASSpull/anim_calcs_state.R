...
if(print_tags == 1){
  print("CreateInputsSubs_CBW/NASSpull/anim_calcs.R")
}
...
stateFileNames = readLines("CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData/State_FileNames.txt")
state_commodities_list=gsub("CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData/","",gsub(".txt","",stateFileNames[2:length(stateFileNames)]))

###beef and dairy
#(1) fattened cattle (uses data from `CATTLE, ON FEED - SALES FOR SLAUGHTER, MEASURED IN HEAD`)
fattened_cattle_sold_list = grep("CATTLE_ON_FEED_SALES", state_commodities_list, value = TRUE)
fattened_cattle_sold = array(0,c(length(NASS_State[,1]),length(fattened_cattle_sold_list)))
for(i in 1:length(fattened_cattle_sold_list)){
  fattened_cattle_sold[,i] = as.numeric(NASS_State[, fattened_cattle_sold_list[i]])
}
#dynamic calculation
...
#(2) milk cows (uses data from `CATTLE, COWS, MILK - INVENTORY`)
milk_cows_list = grep("CATTLE_COWS_MILK_INV", state_commodities_list, value = TRUE)
milk_cows = array(0,c(length(NASS_State[,1]),length(milk_cows_list)))
for(i in 1:length(milk_cows_list)){
  milk_cows[,i] = as.numeric(NASS_State[, milk_cows_list[i]])
}

beef_cows_list = grep("CATTLE_COWS_BEEF_INV", state_commodities_list, value = TRUE)
other_cattle_list = grep("EXCL_COWS", state_commodities_list, value = TRUE)
cattle_sold_list = grep("CATTLE_GE_500_LBS_SALES", state_commodities_list, value = TRUE)
calves_sold_list = grep("CATTLE_CALVES_SALES", state_commodities_list, value = TRUE)
beef_cows = array(0,c(length(NASS_State[,1]),length(beef_cows_list)))
other_cattle = array(0,c(length(NASS_State[,1]),length(other_cattle_list)))
cattle_sold = array(0,c(length(NASS_State[,1]),length(cattle_sold_list)))
calves_sold = array(0,c(length(NASS_State[,1]),length(calves_sold_list)))
for(i in 1:length(beef_cows_list)){
  beef_cows[,i] = as.numeric(NASS_State[, beef_cows_list[i]])
  other_cattle[,i] = as.numeric(NASS_State[, other_cattle_list[i]])
  cattle_sold[,i] = as.numeric(NASS_State[, cattle_sold_list[i]])
  calves_sold[,i] = as.numeric(NASS_State[, calves_sold_list[i]])
}
...
hogs_and_pigs_list = grep("HOGS_INV", state_commodities_list, value = TRUE)
hogs_for_breeding_list = grep("HOGS_BREEDING_INV", state_commodities_list, value = TRUE)
hogs_sales_list = grep("HOGS_SALES", state_commodities_list, value = TRUE)
hogs_and_pigs = array(0,c(length(NASS_State[,1]),length(hogs_and_pigs_list)))
hogs_for_breeding = array(0,c(length(NASS_State[,1]),length(hogs_for_breeding_list)))
hogs_sales = array(0,c(length(NASS_State[,1]),length(hogs_sales_list)))
for(i in 1:length(hogs_and_pigs_list)){
  hogs_and_pigs[,i] = as.numeric(NASS_State[, hogs_and_pigs_list[i]])
  hogs_for_breeding[,i] = as.numeric(NASS_State[, hogs_for_breeding_list[i]])
  hogs_sales[,i] = as.numeric(NASS_State[, hogs_sales_list[i]])
}
#dynamic calculation
...
layers_list = grep("CHICKENS_LAYERS_INV", state_commodities_list, value = TRUE)
pullets_inv_list = grep("CHICKENS_PULLETS_REPLACEMENT_INV", state_commodities_list, value = TRUE)
pullets_sales_list = grep("CHICKENS_PULLETS_REPLACEMENT_SALES", state_commodities_list, value = TRUE)
broilers_inv_list = grep("CHICKENS_BROILERS_INV", state_commodities_list, value = TRUE)
broilers_sales_list = grep("CHICKENS_BROILERS_SALES", state_commodities_list, value = TRUE)
layers = array(0,c(length(NASS_State[,1]),length(layers_list)))
pullets_inv = array(0,c(length(NASS_State[,1]),length(pullets_inv_list)))
pullets_sales = array(0,c(length(NASS_State[,1]),length(pullets_sales_list)))
broilers_inv = array(0,c(length(NASS_State[,1]),length(broilers_inv_list)))
broilers_sales = array(0,c(length(NASS_State[,1]),length(broilers_sales_list)))
for(i in 1:length(layers_list)){
  layers[,i] = as.numeric(NASS_State[, layers_list[i]])
  pullets_inv[,i] = as.numeric(NASS_State[, pullets_inv_list[i]])
  pullets_sales[,i] = as.numeric(NASS_State[, pullets_sales_list[i]])
  broilers_inv[,i] = as.numeric(NASS_State[, broilers_inv_list[i]])
  broilers_sales[,i] = as.numeric(NASS_State[, broilers_sales_list[i]])
}
#dynamic calculation
...
turkeys_list = grep("TURKEYS_INVENTORY", state_commodities_list, value = TRUE)
turkeys_sold_list = grep("TURKEYS_SALES", state_commodities_list, value = TRUE)
turkeys = array(0,c(length(NASS_State[,1]),length(turkeys_list)))
turkeys_sold = array(0,c(length(NASS_State[,1]),length(turkeys_sold_list)))
for(i in 1:length(turkeys_list)){
  turkeys[,i] = as.numeric(NASS_State[, turkeys_list[i]])
  turkeys_sold[,i] = as.numeric(NASS_State[, turkeys_sold_list[i]])
}
#dynamic calculation
...
#(17) sheep
sheep_list = grep("SHEEP", state_commodities_list, value = TRUE)
sheep = array(0,c(length(NASS_State[,1]),length(sheep_list)))
for(i in 1:length(sheep_list)){
  sheep[,i] = as.numeric(NASS_State[, sheep_list[i]])
}

#(18) horses
horses_list = grep("EQUINE", state_commodities_list, value = TRUE)
horses = array(0,c(length(NASS_State[,1]),length(horses_list)))
for(i in 1:length(horses_list)){
  horses[,i] = as.numeric(NASS_State[, horses_list[i]])
}

#(19) goats
goats_angora_list = grep("GOATS_ANGORA", state_commodities_list, value = TRUE)
goats_milk_list = grep("GOATS_MILK", state_commodities_list, value = TRUE)
goats_angora = array(0,c(length(NASS_State[,1]),length(goats_angora_list)))
goats_milk = array(0,c(length(NASS_State[,1]),length(goats_milk_list)))
for(i in 1:length(goats_angora_list)){
  goats_angora[,i] = as.numeric(NASS_State[, goats_angora_list[i]])
  goats_milk[,i] = as.numeric(NASS_State[, goats_milk_list[i]])
}
...
NASS_state_anim_sums = data.frame(anim_sums_array)
rownames(NASS_state_anim_sums) <- animalnames

animpoptotal_state = array(0,c(n_meats,length(import_yrs)))

animpoptotal_state[1,] = colSums(fattened_cattle_sold)+colSums(expected_beef_calves)+colSums(beef_stockers_sold)+colSums(beef_stockers_inventory)
animpoptotal_state[2,] = NASS_state_anim_sums[2,]

#full county level animal average inventories for all years
anim_avg_inv_array = array(0,c(length(NASS_State[,1]),19,length(year)))
for(n in 1:(length(year))){
  ...
  anim_avg_inv_array[,,n]=array(anim_avg_inv,c(length(NASS_State[,1]),19))
