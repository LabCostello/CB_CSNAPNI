...
if(print_tags == 1){
  print("CreateInputsSubs_CBW/NASSpull/NASS_states.R")
}

write(paste("Error Log:",Sys.time()), file = "CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData/Error_Log.txt",
      append = FALSE, sep = "/n")

write("Names of NASS data files generated that are to be used in commodity calculations:", file = "CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData/State_FileNames.txt",
      append = FALSE, sep = "/n")
        ...
        str_replace_all(
          str_replace_all(
            str_replace_all(
              str_replace_all(
                query_desc[j], " ", "_"), ",", ""), "-", ""), "__", "_"), "\\(",""), "\\)","")
    file_name = paste("CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData/",short_desc,'_',year[i],data_source,note,".txt",sep='')
    if (file.exists(file_name)){
      #print(paste("****************** already have:",file_name))
      write(file_name, file = "CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData/State_FileNames.txt",
            append = TRUE, sep = "/n")
    }else{
      ...
      tryCatch({
        ...
        write(states_list, file = "CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData/StateNames.txt", append = TRUE, sep = "/n")
        write(file_name, file = "CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData/State_FileNames.txt",
              append = TRUE, sep = "/n")
      }, warning = function(w) {
        ...
        write(error_desc, file = "CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData/Error_Log.txt",
              append = TRUE, sep = "/n") 
      }, finally = {
        #cleanup-code
      })
...
#list exceptions
survey_not_census = c(list.files(path = "CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData", pattern = "SURVEY", all.files = FALSE,
                                 full.names = FALSE, recursive = FALSE,
                                 ignore.case = FALSE, include.dirs = FALSE, no.. = FALSE))
other_year = c(list.files(path = "CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData", pattern = "using", all.files = FALSE,
                          full.names = FALSE, recursive = FALSE,
                          ignore.case = FALSE, include.dirs = FALSE, no.. = FALSE))
...
write("Used survey data instead of census (state-level census was not available):", file = "CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData/Exceptions.txt",
      append = FALSE, sep = "/n")
write(survey_not_census, file = "CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData/Exceptions.txt",
      append = TRUE, sep = "/n")
write(" ", file = "CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData/Exceptions.txt",
      append = TRUE, sep = "/n")
write("Used data from the nearest available year (data from desired year was not available):", file = "CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData/Exceptions.txt",
      append = TRUE, sep = "/n")
write(other_year, file = "CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData/Exceptions.txt",
      append = TRUE, sep = "/n")

#Get a list of all the data files
FileNames = readLines("CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData/State_FileNames.txt")
state_commodities_list=gsub("CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData/","",gsub(".txt","",FileNames[2:length(FileNames)]))
#Get a list of all the states from which we have data
NASS_states = data.frame(unique(readLines("CreateInputsSubs_CBW/NASSpull/RawNASSData/StateData/StateNames.txt")))
