...
for(n in 1:length(year_labels)){
  ...
  cropws <- data.frame("FIPS"=dummy$FIPS,
                       ...
                       "REGION"=dummy$REGION,
                       "Corn.grain" = dummy$Corn*dummy$`corn for grain`,
                       "Corn.silage" = dummy$Corn*dummy$`corn for silage`,
                       ...
                       "Soybeans" = dummy$Soybeans*dummy$soybeans,
  ...
  cropws <- cropws %>% mutate(Grass=Corn.grain*jessgresults$percentage,
                              Corn.grain=Corn.grain*(1-(jessgresults$percentage)),
                              WinterRye=(Corn.grain+Corn.silage)*wr_adoption_corn) 
...
for(n in 1:(length(year_labels))){ 
  ...
  cropws <- cropws %>% mutate(Corn.grain=Corn.grain*(1-(jessgresults$percentage))*0.8,Soybeans=Soybeans*(1-(jessgresults$percentage))*0.2,Grass=Corn.grain*jessgresults$percentage)
